...
//            System.out.println(fileInputAbsolutePath);
    ...
    private static List<Meta.FileConfig.FileInfo> makeFileTemplates(TemplateMakerFileConfig templateMakerFileConfig,
        ...
        // 如果是文件组
        TemplateMakerFileConfig.FileGroupConfig fileInfoConfig = templateMakerFileConfig.getFileGroupConfig();
        if (fileInfoConfig != null) {
            String condition = fileInfoConfig.getCondition();
            String groupKey = fileInfoConfig.getGroupKey();
            String groupName = fileInfoConfig.getGroupName();
    ...
    private static List<Meta.FileConfig.FileInfo> distinctFiles(List<Meta.FileConfig.FileInfo> fileInfoList) {
        // 1. 将所有文件配置（fileInfo）
        // 2. 对于有分组的文件配置，如果有时间的分组
        // 3. 创建新的文件配置列表（结果列表），先将合并后的分组添加到结果列表
        // 4。再将无分组的文件配置列表添加到结果列博熬
    ...
    public static void main(String[] args) {
        ...
        String projectPath = System.getProperty("user.dir");
        String originalProjectPath = new File(projectPath).getParent() + File.separator + "y-generator-demo-project/springboot-init";
        String inputFilePath1 = "src/main/java/cn/y/springbootinit/common";
//        String inputFilePath2 = "src/main/java/cn/y/springbootinit/controller";
